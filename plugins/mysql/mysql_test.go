...
func TestBuildWhereMySQLTypedFilterValues(t *testing.T) {
	filterValues, err := plugin.NewValueMap(map[string]interface{}{
		"active": true,
		"id":     float64(1),
		"name":   nil,
	})
	...
	where, args, err := buildWhereMySQL(filterValues, "")
	if err != nil {
		t.Fatalf("buildWhereMySQL: %v", err)
	}
	if where != "`active`=? AND `id`=? AND `name` IS NULL" {
		t.Fatalf("where = %q", where)
	}
	if len(args) != 2 || args[0] != true || args[1] != float64(1) {
