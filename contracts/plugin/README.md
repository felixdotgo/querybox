...
- Resource graph nodes accept action objects where the `type` field is a machine name such as `"select"` or `"describe"`. Go authors can use the constants defined in `pkg/plugin/resource_graph.go` (for example `plugin.ResourceActionSelect`) instead of hardcoding the strings.
